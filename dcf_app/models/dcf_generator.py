...
def run_dcf_from_inputs(inputs, wacc=0.10, terminal_growth=0.03, exit_multiple=None):
    """
    Complete DCF valuation from raw inputs using centralized discounted_cash_flow().
    """
    required_keys = [
        'revenue_base',
        'revenue_growth',
    ...
    value, terminal_info = discounted_cash_flow(
        fcfs,
        wacc=wacc,
        terminal_growth=terminal_growth,
        exit_multiple=exit_multiple,
        method="exit" if exit_multiple else "perpetuity"
    )
    print("📈 FCFs:", fcfs)
    ...
    return value, {
        "method": "exit" if exit_multiple else "perpetuity",
        "terminal_value": terminal_info.get("terminal_value", 0),
        "exit_multiple": exit_multiple,
        "final_fcf": fcfs[-1] if fcfs else 0,
        "discounted_terminal_value": terminal_info.get(
